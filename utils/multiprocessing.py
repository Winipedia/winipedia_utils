...
def cancel_on_timeout_with_multiprocessing(seconds: int, message: str) -> Callable[[Callable[[Any, Any], Any]], Callable[[Any, Any], Any]]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            with PoolThreadPool(processes=1) as pool:
                ...
                try:
                    ...
                    logger.warning(f"{func.__name__} -> Execution exceeded {seconds} seconds: {message}")
...
def multithread_loop(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        if max_workers > 1:
            results = [executor.submit(_process_function_for_imap_with_args_unpacking, process_args_single) for process_args_single in process_args]
            results = _get_future_results_as_completed(results)
        else:
            results = map(_process_function_for_imap_with_args_unpacking, process_args)

        return _get_multiprocess_results_with_tqdm(
            results=results,
            process_func=process_function,
            process_args=process_args,
            threads=True,
...
def multiprocess_loop(
    ...
    with Pool(processes=max_processes) as pool:
        if max_processes > 1:
            ...
            results = pool.imap_unordered(_process_function_for_imap_with_args_unpacking, process_args)
        else:
...
def _prepare_multiprocess_loop(
    ...
    max_pools = _find_max_pools_for_multiprocessing(threads=threads, length_process_args=len(process_args[0]))
    ...
    process_args = list(zip(*process_args, strict=False))
...
def _find_max_pools_for_multiprocessing(threads: bool, length_process_args: int) -> int:
    ...
    logger.info(f"Multi{'threading' if threads else 'processing'} with max_pools: {max_pools}")
...
def _extend_process_args_with_const_args(
    ...
    for process_arg_static in process_args_static:
        if deepcopy_static_args:
            process_args_static_as_list = [copy.deepcopy(process_arg_static) for _ in range(length_process_args)]
        else:
