...
class BaseTestCaseForFile(TestCase, ABCImplementationLoggingMixin):
    ...
    def test_tested_file_is_correct_type(self) -> None:
        assert inspect.ismodule(self.tested_file)

    def test_correct_test_cls_name(self) -> None:
        module_name = self.tested_file.__name__.split(".")[-1]
        if module_name == "__main__":
            msg = "The module name cannot be __main__"
            raise ValueError(msg)
        supposed_class_name = self.make_test_case_cls_name(module_name)
        actual_class_name = self.__class__.__name__
        assert supposed_class_name == actual_class_name
    ...
    def make_test_case_cls_name(file_name: str) -> str:
        ...
        return "".join([s.capitalize() for s in supposed_class_name])

    def test_correct_test_file_path(self) -> None:
        ...
        assert supposed_test_file_path == test_file_path
    ...
    def make_test_path(file_path: str) -> str:
        ...
        return os.path.join(test_folder_name, test_path)

    def test_all_functions_in_file_tested(self) -> None:
        ...
        functions_not_tested = [self.make_test_func_name(f) for f in functions if self.make_test_func_name(f) not in test_functions]

        assert not functions_not_tested, self._get_untested_summary_error_msg(untested_functions=functions_not_tested)
    ...
    def test_all_class_methods_tested(self) -> None:
        """Ensure all methods of user-defined classes in the file are tested."""
        # Get all user-defined classes in the file
        cls_methods: dict[type, list[str]] = self.get_all_cls_and_cls_methods_from_module(file=self.tested_file)
        ...
        for cls, methods in cls_methods.items():
            ...
            untested = [
                ...
                if self.make_cls_method_test_func_name(cls, method) not in test_functions
            ]
        ...
        assert not untested_methods, self._get_untested_summary_error_msg(untested_cls_methods=untested_methods)
    ...
    def get_all_cls_and_cls_methods_from_module(
        ...
        for cls in classes:
            ...
            methods = [
                ...
                if (
                    (inspect.isfunction(method) or inspect.ismethod(method))  # Include instance, static, and class methods
                    and method.__module__ == file.__name__  # Ensure it's from the target module
    ...
    def _get_untested_summary_error_msg(
        ...
        if untested_cls_methods is None and untested_functions is None:
            msg = "Either untested_cls_methods or untested_functions must be provided."
            raise ValueError(msg)
